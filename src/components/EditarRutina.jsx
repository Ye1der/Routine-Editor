...
export function EditarRutina(){
  ...
  const {usuario, setEditar, editar} = useContext(contextGlobal)
  ...
  async function onSubmit(data){ // La funcion handleSubmit ya le proporciona el parametro data
    ...
    spliceArray.splice(editar, 1, objectRutine)
    ...
    <div className='absolute z-20 flex items-center justify-center bg-black bg-opacity-80 w-full h-full'>
      ...
      <form className='flex flex-col overflow-auto w-[90%] m-auto'
        onSubmit={handleSubmit(onSubmit)}>
          ...
          <input className='text-white text-4xl font-bold text-opacity-85 my-3 w-4/5 outline-none bg-transparent ml-4 transition-all duration-200 placeholder:opacity-40 placeholder:font-bold caret-white cursor-text focus:placeholder:opacity-70' // Input para el nombre de la rutina
