    ...
    return (
        <main className="w-full h-screen flex flex-col items-center justify-center " >
            <div className="bg-grayGym bg-opacity-50 w-96 rounded-3xl" >
                ...
                <h1 className="text-white text-4xl text-opacity-80 mt-6 ml-8 font-bold" > Crear cuenta </h1>
                ...
                <div className="w-full mt-8" >
                    ...
                    autoComplete='off'
                    type="text" placeholder="Digite la direccion de correo" id="Email"/>
                </div>
                ...
                <div className='w-full mt-8' >
                    ...
                    <input className=" text-white text-opacity-75 w-4/5 outline-none bg-transparent border-b-white h-8 ml-8 border-b-2 border-white border-opacity-50 transition-all duration-200 placeholder:opacity-40 caret-white cursor-text focus:border-opacity-90" 
                    onChange={()=>{setContraseña1(event.target.value)}}
                    type="password" placeholder="Contraseña" id='contraseña'/>
                </div>

                <div className='w-full mt-8' >
                    <label className="text-white font-bold text-lg text-opacity-90 ml-8 cursor-text flex" 
                    htmlFor="contraseña2"> <HiLockClosed className='relative top-1 mr-1'/> Contraseña </label>
                    <input className=" text-white text-opacity-75 w-4/5 outline-none bg-transparent border-b-white h-8 ml-8 border-b-2 border-white border-opacity-50 transition-all duration-200 placeholder:opacity-40 caret-white cursor-text focus:border-opacity-90"  
                    onChange={()=>{setContraseña2(event.target.value)}}
            ...
            {validador === 3 ? <h1  className='text-red-500 text-opacity-90 text-center font-bold mt-3'  > Direccion de correo electronico no valida </h1> : "" }

            {validador === 5 ? <h1  className='text-red-500 text-opacity-90 text-center font-bold mt-3'  > La contraseña debe tener mas de 6 caracteres </h1> : "" }

            {validador === 2 ? <h1 className='font-bold text-green-300 text-center mt-3'> La cuenta a sido creada con exito <br /> ahora puede 
            <a onClick={()=>{navigate("/login")}} className='cursor-pointer text-emerald-400 hover:text-emerald-500 duration-300' > iniciar sesion </a></h1> : ""}

            {validador === 4 ? <h1  className='text-red-500 text-opacity-90 text-center font-bold mt-3'  > Esta direccion de correo electronico ya existe, 
            <br/> <a onClick={()=>{navigate("/login")}} className='cursor-pointer text-red-400 hover:text-red-300 duration-300' > inicie sesion </a> o use otro correo</h1> : ""}
